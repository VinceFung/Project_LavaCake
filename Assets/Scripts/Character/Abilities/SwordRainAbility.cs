...
public class SwordRainAbility : MonoBehaviour
{
    ...
    public void CastSwordRain()
    {
        ability.Owner.OnAbilityCast.Invoke();

        foreach (Transform spawnPoint in spawnPoints)
        {
            StartCoroutine(SpawnField(spawnPoint.position, Random.Range(0f, 1f)));
        }
    }

    IEnumerator SpawnField(Vector3 spawnPos, float delay)
    {
        ...
        GameManager.SpawnExplosion(spawnPos, explosionRadius, explosionRadiusNear, explosionRadiusFar, damageFallOff, dmg, debuffToApply, ability.Owner);
        ...
        foreach (DamageField field in DamageFields.ToArray())
        {
            if (field.duration > 0f)
            {
                ...
                if (Time.time >= field.nextTimeToTick)
                {
                    ...
                    GameManager.SpawnExplosion(field.pos, explosionRadius, explosionRadius, explosionRadius, 1f, dmg, debuffToApply, ability.Owner);
                }
            }
            else
            {
